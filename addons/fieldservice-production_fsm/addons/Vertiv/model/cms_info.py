...
class cms_info(models.Model):
    ...
    new_call = fields.Integer('New call', default=0)
    to_notify = fields.Integer('To Notify', default=0)
    notification_sent = fields.Boolean('Notification Sent')
    # risk_assessement_ids = fields.One2many('risk.assessement', 'call_id', 'Risk Assessement Data',
    #                                        compute="compute_risk_assessement_ids")
    call_origin = fields.Char('Call Origin')

    def rejected_call_notification_automated_action(self):
        call_id = self.env['cms.info.model'].sudo().search([('id', '=', self._context.get("active_id"))])
        if call_id.call_status == 'Unassigned' and call_id.engineerId:
            notify = self.env['notification']
            body = "Call No" + call_id.call_no + " has been Rejected"
            vals = {'notification_type': 'Email', 'notification_from': 'FSM', 'body': body,
                    'subject': 'Call No ' + call_id.call_no + ' is Rejected',
                    'notification_to': call_id.engineerId.parent_id.work_email, 'sent_time': datetime.now()}
            notify.sudo().create(vals)

    def forwarding_call_notification_automated_action(self):
        call_id = self.env['cms.info.model'].sudo().search([('id', '=', self._context.get("active_id"))])
        if call_id.call_status == 'Open' and call_id.engineerId:
            notify = self.env['notification']
            body = "Call No" + call_id.call_no + " has been Forwarded to This Engineer" + call_id.engineerId.name
            vals = {'notification_type': 'Email', 'notification_from': 'FSM', 'body': body,
                    'subject': 'Call No ' + call_id.call_no + ' has been Forwarded',
                    'notification_to': call_id.engineerId.parent_id.work_email, 'sent_time': datetime.now()}
            notify.sudo().create(vals)
    ...
    def _get_response_time(self):
        for record in self:
            if record.call_actual_startdate and record.call_log_date:
                duration = record.call_actual_startdate - record.call_log_date
                d_sec = duration.total_seconds()
    ...
    def change_engineer(self):
        ...
        if (self.product_model and self.product_model!='0'):
            if self.product_model.split('.')[0]=="POWER":
                model = self.product_model.split('.')[1]
            else:
                model=self.product_model
        else:
            model=self.product_model
        _logger.info(model)
        product_model = self.env['product.model'].sudo().search([('model','=',model)],limit=1)
    ...
    def newcall_notification(self):
        calls = self.env['cms.info.model'].sudo().search([('engineerId', '!=', False),('to_notify', '=', 1)])
        c = self.env['collab']
        for i in calls:
            email = i.engineerId.work_email
            pm = c.post_msg(email, 'New call no ' + str(i.call_no) + ' assigned to you')
            if pm and pm.status_code == 200:
                i.to_notify = 0
            if i.customer_latitude or i.customer_longitude:
                i.call_origin = 'Vertiv APP'
    ...
    @api.model
    def on_demand_gen_report(self, call_list=False):

        current_time = datetime.today().date()
        if call_list:
            calls = self.search([('call_no', 'in', call_list)])
        else:
            calls = self.env['cms.info.model'].search(
                ['&', '&', '&', ('write_date', '>=', current_time), ('call_status', '=', 'Completed'), '|',
                 ('field_service_report_no', '!=', 'Failed'),
    ...
    @api.model
    def on_demand_gen_report_record(self, call_list=False):
...
class CallHistory(models.Model):
    _name = 'call.history'
    _rec_name = 'call_no'
    _description = 'Maintain all status and engineer changes'
